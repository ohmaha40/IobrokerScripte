...
//Variable setzen
var timeout;
var standby_power = 10;                                                                             //Leistung die überschritten werden muss
...
on({id: 'javascript.0.Trockner.ON'/*Trockner ON*/, change: "ne"}, function(obj){
    if(getState('javascript.0.Trockner.ON'/*Trockner ON*/).val == true){
        sendTo("telegram.0", "send",{text: ('Die Trockner läuft jetzt!'),disable_notification: true});
    }
    else {
        sendTo("telegram.0", "send",{text: ('Die Trockner ist fertig!'),disable_notification: true});
    }
});
